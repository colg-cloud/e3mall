package cn.e3mall.common.freemarker;
...
public class FreemarkerTest {
    ...
    public void testFreemarkerHtml() throws Exception {
        ...
        configuration.setDirectoryForTemplateLoading(new File(pathname));
        // 3. 创建一个数据集
        Student student = new Student(1, "Tom", 20, "南美");
        List<Student> list = CollUtil.newArrayList(
            new Student(2, "Jack", 18, "美国"),
            new Student(3, "Rose", 20, null)
        );
        Dict data = Dict.create().set("stuList", list)
                                 .set("student", student)
                                 .set("date", new Date())
                                 .set("val", null)
